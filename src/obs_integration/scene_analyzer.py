...
class SceneAnalyzer:
    ...
    def analyze_scene_sources(self, scene_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            def enum_scene_items(scene_obj, scene_item, data):
                ...
                final_width = (
                    int(source_width * scale.x) if source_width > 0 else int(bounds.x)
                )
                final_height = (
                    int(source_height * scale.y) if source_height > 0 else int(bounds.y)
                )
                ...
                sources[source_name] = {
                    ...
                    "bounds": {"x": float(bounds.x), "y": float(bounds.y)},
                    "dimensions": {
                    ...
                    "transform": {
                        ...
                        "bounds_alignment": getattr(
                            transform_info, "bounds_alignment", 0
                        ),
                    },
                    "visible": True,
    ...
    def save_scene_metadata(
        self, output_path: str, scene_name: Optional[str] = None
    ) -> bool:
        """
